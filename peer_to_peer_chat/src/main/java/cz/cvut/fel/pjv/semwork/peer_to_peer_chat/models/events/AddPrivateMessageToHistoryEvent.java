...
/**
 * This adds message to private message history
 * @Accepts Pair<Peer,Pair<Peer,Message>>
 *     Pair of (Target peer, Pair of (Sender peer [local or target], Message))
 */
public class AddPrivateMessageToHistoryEvent extends Event<Pair<Peer,Pair< Peer,Message>>, Void> {
    public AddPrivateMessageToHistoryEvent() {
        this.receivers = new ArrayList<>(Arrays.asList(
                EventReceiver.MESSAGE_SERVICE,
                EventReceiver.UI
        ));
        this.sender = EventSender.API;
        this.type = EventType.ADD_PRIVATE_MESSAGE_TO_HISTORY;
    }
}
